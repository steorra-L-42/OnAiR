...
class KafkaConsumerWrapper:
  ...
  def consume_messages(self):
    ...
    while not self.stop_event.is_set():
      ...
      try:
        ...
        # poll 단계에서 에러 처리
        if msg.error():
          log.error(f"Consumer error: {msg.error().str()}")
        ...
        log.error(f"Unhandled error during polling: {e}")
        if msg:
          self.send_to_dlt(msg)
  ...
  def process_message(self, msg):
    ...
    try:
      log.info(f"Processing message: {msg.value().decode('utf-8')}")
  ...
  def send_to_dlt(self, msg):
    ...
    if self.create_topic_if_not_exists(dlt_topic):
      log.info(f"Send Record to {dlt_topic}, Key : {msg.key()}, Value : {msg.value()}")
