...
# 내부 패키지: 설정
from config import STREAMING_CH_DIR, hls_output_dir
...
async def serve_playlist(stream_name: str):
  playlist_path = os.path.join(STREAMING_CH_DIR, stream_name, hls_output_dir, "index.m3u8")
...
async def serve_segment(stream_name: str, segment: str):
  segment_path = os.path.join(STREAMING_CH_DIR, stream_name, hls_output_dir, segment)
